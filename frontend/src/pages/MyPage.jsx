...
// ==========================================
// 0. 유틸리티 및 STOMP/Flask 헬퍼 함수
// ==========================================

// STOMP/Flask 구독 관리를 위한 전역 헬퍼 함수
...
const getColor = (rate) => rate > 0 ? '#d60000' : rate < 0 ? '#0051c7' : '#333';

// ==========================================
// 1. 내부 컴포넌트: 심플 워드 클라우드 (원본 유지)
// ==========================================
...
function MyPage() {
    ...
    // ⭐ 실시간 데이터와 DB 데이터를 결합하는 로직
    const combinedStocks = favorites.stocks.map(stock => {
        const rtData = rtStockData[stock.stockCode];
        const changeRate = Number(rtData?.changeRate ?? stock.changeRate);
        const price = rtData?.currentPrice ?? stock.price;
        ...
        return {
            ...
            displayPrice: price ? price.toLocaleString() : '-',
            displayRate: formatChangeRate(changeRate),
            color: getColor(changeRate)
        };
    });
                ...
                <>
                    ...
                    <div style={styles.card}>
                        {combinedStocks.length === 0 ? <p style={{color:'#888'}}>찜한 종목이 없습니다.</p> : 
                            combinedStocks.map((stock, idx) => (
                                <div key={stock.stockCode || idx} style={styles.listItem}>
                                    <div style={{flex: 1}}>
                                    ...
                                    <div style={{display:'flex', flexDirection: 'column', alignItems:'flex-end', gap:'5px', minWidth: '100px'}}>
                                        <span style={styles.stockPrice(stock.changeRate)}>
                                            {stock.displayPrice}원
                                        </span>
                                        <span style={{ ...styles.stockPrice(stock.changeRate), fontSize: '14px' }}>
                                            {stock.displayRate}
                                        </span>
                    ...
                    <div style={styles.card}>
                        {favorites.news.length === 0 ? <p style={{color:'#888'}}>스크랩한 뉴스가 없습니다.</p> : 
                            favorites.news.map((news, idx) => {
                                ...
                                return (
                                    <div key={news.newsId || idx} style={styles.listItem}>
